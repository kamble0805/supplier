...
All notable changes and architectural updates to this project are documented below.

### [2026-09-05] - Repository Sanitization & Output Cleanup (`db27917`)
* **Removed Generated Outputs**: Removed all script-generated Excel reports from version control and disk to keep the repository strictly scoped to inputs:
  * `template_PDF_Extracted.xlsx` (generated by `extract_po_data.py`)
  * `supplier_comparison_report.xlsx` (generated by `compare_suppliers.py`)
  * `all_po_details.xlsx` (generated by `extract_all_po_details.py`)
  * `item/ItemMaster_Reconciled.xlsx` (generated by `item/verify.py`)
* **Purged Runtime Artifacts & Caches**:
  * Removed checked-in Python bytecode cache files (`__pycache__/`, `item/__pycache__/`).
  * Removed execution log file (`item/po_reconciliation.log`).
* **Added Git Hygiene Configuration**:
  * Added `.gitignore` to prevent future commits of `.pyc` files, runtime logs, temporary MS Excel lock files (`~$*.xlsx`), and generated output spreadsheets.
* **Result**: Repository sanitized to pure inputs only (raw PDFs, master database files, schemas, and processing scripts).

### [2026-09-05] - Technical Architecture & Logic Documentation (`deb7b3a`)
* **Added Comprehensive Documentation (`README.md`)**:
  * Added Mermaid architectural data flow diagram for all 3 data pipelines.
  * Documented coordinate-based word reconstruction logic ($\Delta y \le 2.0\text{px}$) for PDF extraction.
  * Documented regular expression patterns for GSTIN, PAN, contacts, delivery modes, and financial line items.
  * Documented the 8-stage false-positive rejection pipeline for Item Master verification.
  * Added reference schema breakdown for master CSV and Excel datasets.
  * Included end-to-end execution guide and troubleshooting for Excel permission locks.

### [2026-09-05] - Initial Project Setup (`d810713`)
* **Added Core Processing Scripts**:
  * `extract_po_data.py`: Vendor header extraction and ERP template alignment.
  * `compare_suppliers.py`: Supplier master reconciliation script.
  * `extract_all_po_details.py`: Full line-item and spend transaction parser.
  * `item/verify.py`: Item code parser and Item Master reconciliation engine.
* **Added Master Data & Templates**:
  * `template.xlsx`: 15-column empty ERP supplier onboarding template.
  * `supplierpmtrack.csv`: Master database containing 553 suppliers.
  * `Supplier_master.xlsx`: Master database containing 464 suppliers with status tags.
  * `item/item master.csv`: Master part/item database containing catalog items.
* **Added Raw Ingestion Data**:
  * Added 221 Purchase Order PDFs from July and August 2026 under `pdfs/`.
